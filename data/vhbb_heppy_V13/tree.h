...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Int_t           TauGood_idMVA[12];   //[nTauGood]
   Int_t           TauGood_idMVANewDM[12];   //[nTauGood]
   Int_t           TauGood_idCI3hit[12];   //[nTauGood]
   Int_t           TauGood_idAntiMu[12];   //[nTauGood]
   Int_t           TauGood_idAntiE[12];   //[nTauGood]
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("tree_M-260.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("tree_M-260.root");
      }
