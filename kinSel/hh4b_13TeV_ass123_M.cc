...
int nJets_cut = 4; //3
double Jet_pt_cut_high = 90.;
double Jet_eta_cut = 10; //2.5 - 10=noCut
double CSV_cut_med = 0.679; //0.679
...
//   MAIN
//-------------
void hh4b_nonres_match123_M(std::string sample, std::string opt, double maxEvents =0, bool readmatrix=false)
{
  ...
  // Book variables (debug - check which are useless)
  double Vtype_;
  int evt, run;
  ...
  double GenH_pt[2], GenH_mass[2], GenBfH_pdgId[4],GenBfH_pt[4],GenBfH_eta[4],GenBfH_phi[4],GenBfH_mass[4];
  double GenBfHafterISR_pdgId[4],GenBfHafterISR_pt[4],GenBfHafterISR_eta[4],GenBfHafterISR_phi[4],GenBfHafterISR_mass[4];
  double weightPU, HLT_HH4bDouble, HLT_HH4bQuad;
  ...
  tree->SetBranchAddress("HLT_HH4bQuad",&HLT_HH4bQuad);     
  tree->SetBranchAddress("HLT_HH4bDouble",&HLT_HH4bDouble);     
...
//  TH1F *h_nCand_true=new TH1F("h_nCand_true", "# HH candidates if true", 20, 0., 20.);	
  TH1F *h_nPV=new TH1F("h_nPV", "# of Primary Vertices; nPV", 10, 0., 10.); h_nPV->Sumw2();
  TH1F *h_nPV_weighted=new TH1F("h_nPV_weighted", "# of Primary Vertices after Reweighting; nPV", 50, 0., 50.); h_nPV_weighted->Sumw2();
...
//  TH2F *h_R_CSVeta = new TH2F("h_R_CSVeta", " R; CSV; eta)", 50, 0., 1., 100, -4., 4.);
  ...
  //output file
  std::string outfilename="tree_"+sample+".root";
  ...
  for (int i=0; i<nEvents; ++i){
   ...
   //debug - emulate trigger cut:
	...
	for (int j=0; j<nJet; ++j){    //loop on all the jets (only inAcc are saved)
        ...
        std::sort (jet.begin(), jet.end(), cmp_CSV );      
        if(jet[2].CSV>CSV_cut_med){ //cut on jets sorted in cmva
  	  ++nCut4;
...
//debug - ..
	if(kbest == 3 && In != 3) {
          ...
          //---------------
            ...
            for(int j=3; j<NJetInAcc; ++j){
              //not optimized...
              if(j==InTrue) for(int n=0;n<8;n++) Ntr[n]++;
              if(j==In && j== kbest && j== 3) { N_all_b[7]++; if(j==InTrue)Nt_all_b[7]++; }
              if(j==In && j== 3) { N_all_b[6]++; if(j==InTrue)Nt_all_b[6]++; }
              if(j==In && j== kbest) { N_all_b[5]++; if(j==InTrue)Nt_all_b[5]++; }
              if(j==In) { N_all_b[4]++; if(j==InTrue)Nt_all_b[4]++; }
              if(j==3 && j== kbest) { N_all_b[3]++; if(j==InTrue)Nt_all_b[3]++; }
              if(j==3) { N_all_b[2]++; if(j==InTrue)Nt_all_b[2]++; }
              if(j==kbest) { N_all_b[1]++; if(j==InTrue)Nt_all_b[1]++; }
              if(j!=kbest && j!=3 && j!=In) { N_all_b[0]++; if(j==InTrue) Nt_all_b[0]++; } 
            }
  ...
  cout << "'EFFICIENCY' (NselectedOK/nMCTruth): "<< endl;
  cout << "nMCTruth " << nMCTruth << endl;
  cout << "RL " << nRmaxOk << " " << (double)nRmaxOk/nMCTruth*100 << endl;
  cout << "RL(nSigmaMaxOk) " << nSigmaMaxOk << " " << (double)nSigmaMaxOk/nMCTruth*100 << endl;
  cout << "CSV " << nCSVOk << " " << (double)nCSVOk/nMCTruth*100 << endl;
  cout << "MinMass " << nMinMassOk << " " << (double)nMinMassOk/nMCTruth*100 << endl;
  cout << endl;
...
/*  cout << "'PURITY' (NselectedOK/NselectedAll): "<< endl;
  ...
  cout << "CSV " << nCSVOk << " " << (double)nCSVOk/nMCTruth*100 << endl;
  cout << "MinMass " << nMinMassOk << " " << (double)nMinMassOk/nMCTruth*100 << endl;
  cout << "RLCSV " << nMatrCSVOk << " " << (double)nMatrCSVOk/nMCTruth*100 << endl;
  cout << endl;*/

//  cout << "'EFFICIENCY' (NselectedOK/nMCTruth): "<< endl;
//  for(int k=0;k<8;k++)  cout << k << " " << Nmatched[k] << " " << Ntr[k] << " " << Nmatched[k]/Ntr[k]*100 << endl;
//  cout << endl;


  cout << "'PURITY' disentagled.. (NselectedOK/NselectedAll): "<< endl;
...
//  h_M_matrixCSV->Draw("same");
 ...
 TCanvas * c5 = new TCanvas("c5","CSVM_4thCSV_b",600,500);
  ...
  std::string histfilename="Histograms_"+sample+"_"+opt+".root";
