...
double CSV_cut = 0.679; //Run2: low 0.605; medium 0.890; high 0.970. Run1: low 0.679 (used in the first presentation).
...
cout << nEvents << endl;
  for (int i=0; i<nEvents; ++i){
   ...
   // ----------------------------------
    if(isSignalMC && i>=0.5*nEvents) {    //to not run on events used to fill the matrix
